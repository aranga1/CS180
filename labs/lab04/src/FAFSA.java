...
public class FAFSA {
	...
	public boolean isFederalAidEligible() {
		if ((age >= 18) && (age <= 25)) {
			if ((isAcceptedStudent == true) && (isSSregistered == true) &&
					   (hasSSN == true) && (hasValidResidency == true)) {
				return true;
			}
			else {
				return false;
			...
			else if ((isAcceptedStudent == true) && (isSSregistered == false) && 
					   (hasSSN == true) && (hasValidResidency == true)) {
				return true;
			}
			else {
			...
			System.out.println("Entered an impossible case. Check method");
			return false;
		}
	}
